...
/**
 * Componente para mostrar un salones individual con funcionalidad de carrito
 *
 * @param {Object} props - Propiedades del componente
 * @param {Object} props.salones - Datos del salones a mostrar
 * @returns {JSX.Element} - Elemento JSX que representa el salones
 */
const Salones = ({ salones }) => {
  console.log(salones)
  ...
  const [stock, setStock] = useState(salones.cantidad);
  ...
  const incrementar = () => {
    if (stock > 0) {
      ...
      setCostoCompra((prev) => prev + salones.precio);
    }
  };
  ...
  const agregarAlCarrito = () => {
    if (cantidad > 0) {
      handleAgregarCarrito(salones, cantidad);
        ...
        {salones.imagen && (
          <div className="h-48 w-full overflow-hidden relative">
            <img
              ...
              src={salones.imagen}
              alt={salones.nombre}
              onClick={() => navigate(`/Salones/${salones.id}`)}
            />
        ...
        <div className="p-4">
          ...
          <div className="flex justify-between items-start mb-2">
            ...
            <span className="text-xl font-bold text-gray-900 whitespace-nowrap">
              ${salones.precio.toLocaleString()}
            </span>
            
            
          </div>
          <h1>{salones.descripcion}</h1>
          {/* Estado de stock */}
          <div className="mb-4">
            <p
